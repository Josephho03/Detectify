...
function Modal({
  ...
  return (
    <div
      ...
      aria-modal="true"
    >
      <div className="w-full max-w-md rounded-2xl border border-zinc-800 bg-zinc-900/80">
...
function DocsModal({
  ...
  return (
    <div
      ...
      aria-modal="true"
    >
      <div className="w-full max-w-lg rounded-2xl border border-zinc-800 bg-zinc-900/90">
        ...
        <div className="p-5">
          <div className="text-zinc-200 max-h-[60vh] overflow-y-auto pr-1">
            {children}
          </div>
          <div className="mt-5 flex justify-end">
            <Button
...
export default function SignupPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const { data, error } = await supabase.auth.signUp({
      ...
      options: {
        // After confirming email, callback will send them to /login (clean URL).
    ...
    if (error) {
      ...
      if (msg.includes("already registered") || msg.includes("user already exists")) {
        openModal(
          ...
          `The email ${formData.email} is already associated with an account.\nTry signing in or use a different email.`
        );
      } else {
    ...
    if (!data.session) {
      openModal(
        "success",
        "Verify your email",
        `We’ve sent a verification link to ${formData.email}.\n\nPlease check your inbox (and spam folder) and click the link to confirm your email. You’ll be redirected back to Detectify to complete your setup.`
      );
      return;
    }
  ...
  const handleOAuth = async (provider: "google" | "github") => {
    ...
    try {
      ...
      await supabase.auth.signInWithOAuth({
        provider,
        options: {
          // Must be allowed in Supabase Auth -> URL config (and Google/GitHub console)
          redirectTo: `${window.location.origin}/auth/callback?next=/`,
          // Optional: help ensure provider prompt shows if a stale session exists
          ...
          <div className="text-center mb-8">
            <Link href="/" className="inline-block mb-6">
              <div className="flex items-center justify-center space-x-2">
                ...
                <span className="text-2xl font-bold text-white">Detectify</span>
              </div>
            </Link>
            <h1 className="text-3xl font-bold text-white mb-2">Create account</h1>
            <p className="text-zinc-400">Join the fight against deepfakes</p>
          </div>
          ...
          >
            <form onSubmit={handleSubmit} className="space-y-6" noValidate>
                  ...
                  {[
                    ...
                    <li key={i} className="flex items-center gap-2">
                      {r.ok ? (
                        ...
                        <CircleX className="h-4 w-4 text-zinc-500" />
                      )}
                      <span className={r.ok ? "text-zinc-300" : "text-zinc-500"}>
                        {r.label}
                      </span>
                    </li>
                  ))}
              ...
              <div className="flex items-start space-x-2">
                ...
                <label htmlFor="terms" className="text-sm text-zinc-300">
                  ...
                  <a
                    ...
                    className="text-[#e78a53] hover:text-[#e78a53]/80 underline underline-offset-2"
                  >
                    Terms of Service
                  </a>{" "}
                  and{" "}
              ...
              <Button
                type="submit"
                disabled={!canSubmit}
                className="w-full bg-[#e78a53] hover:bg-[#e78a53]/90 text-white font-medium py-3 rounded-xl transition-colors disabled:opacity-60"
              >
          ...
          >
            ...
            <div className="mt-6 grid grid-cols-2 gap-3">
              <Button
                ...
                className="bg-zinc-900/50 border-zinc-800 text-zinc-300 hover:bg-zinc-900 hover:border-[#e78a53]/60 transition-all duration-200 group disabled:opacity-60"
              >
                <svg
                  className="w-5 h-5 mr-2 text-zinc-300 group-hover:text-[#e78a53] transition-colors duration-200"
                  viewBox="0 0 24 24"
                >
                  <path
                    fill="currentColor"
                ...
                <span className="group-hover:text-[#e78a53] transition-colors duration-200">
